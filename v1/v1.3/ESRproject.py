...
version = "ESR v1.3\n" \
          "\n" \
          "5.01.24"
description_of_the_version = "Эта программа для обработки пиков ESR.\n" \
                             "\n" \
                             "Загрузка данных:\n" \
                             ...
                             "3. Фитирование пика: правой кнопкой 1 нажатие справа, потом 1 нажатие слева от пика. " \
                             "Между вертикалями будет фитирование пика.\n" \
                             "4.1. Если не устраивает подгонка -> RESET peak.\n" \
                             "4.2. Если после обработки обоих пиков всё устраивает -> Save или Save and Next.\n" \
...
class MainWindow(QMainWindow, Ui_MainWindow, QWidget):
    def __init__(self):
        ...
        self.export_list = {'header': ["F", "#run", " ", "B_peak_mean", "B_peak_DN", "B_peak_UP", " ", "w_half_width_mean", "w_half_width_DN", "w_half_width_UP", " ", "amplitude_peak_mean", "amplitude_peak_DN", "amplitude_peak_UP", " ", "temperature_resistance_mean", "temperature_resistance_DN", "temperature_resistance_UP", " ", "a_DN", "q_DN", "x0_DN", "w_DN", "a_UP", "q_UP", "x0_UP", "w_UP", " ", "a_DN(x3)", "b_DN(x2)", "c_DN(x)", "d_DN(const)", "a_UP(x3)", "b_UP(x2)", "c_UP(x)", "d_UP(const)", " ", "name_of_file_DN", "name_of_file_UP"], 'units': [], 'data': []}
    ...
    def load_data_DN(self):
        ...
        self.data_voc_DN['header'] = ["B", " ", " ", " ", "X", "Y", " ", " ", " ", " ", " ", " ", "R", " ", "t", " ", " "]
    ...
    def peak_fitting_DN(self):
        ...
        y_normalized_data_cut_range = self.y_normalized_cut_full_range_DN
        ...
        for ind in range(len(self.x_data_cut_range_DN)):
            if (self.x_data_cut_range_DN[ind] > self.on_mouse_press_coordinate_list_DN[0][0]) and \
                    (self.x_data_cut_range_DN[ind] < self.on_mouse_press_coordinate_list_DN[1][0]):
                self.x_data_to_peak_fitting_DN.append(self.x_data_cut_range_DN[ind])
                self.y_data_to_peak_fitting_DN.append(y_normalized_data_cut_range[ind])
        ...
        if self.PeakComboBoxDN.currentText() == "Fano resonance":
            ...
            #y_fitting_normalization_around_peak_DN = fitting_function(fano_resonance, self.params_of_peak_DN,self.x_data_to_peak_fitting_DN,plot=False)
            # self.axes_left.plot(self.x_data_to_peak_fitting_DN, y_fitting_normalization_around_peak_DN, color="red", linewidth=2)

            y_fitting_normalization_full_range = fitting_function(fano_resonance, self.params_of_peak_DN,
                                                                      self.x_data_cut_range_DN,
                                                                      plot=False)
            self.axes_left.plot(self.x_data_cut_range_DN, y_fitting_normalization_full_range, color="red", linewidth=2)
    ...
    def load_data_UP(self):
        ...
        # это название колонок в таблице данных
        self.data_voc_UP['header'] = ["B", " ", " ", " ", "X", "Y", " ", " ", " ", " ", " ", " ", "R", " ", "t", " ", " "]
    ...
    def peak_fitting_UP(self):
        """ Дествие почти такое же, как и у background_fitting_UP
        Двумя кликами ограничиваем область для фитирования """

        y_normalized_data_cut_range = self.y_normalized_cut_full_range_UP
        ...
        for ind in range(len(self.x_data_cut_range_UP)):
            if (self.x_data_cut_range_UP[ind] > self.on_mouse_press_coordinate_list_UP[0][0]) and \
                    (self.x_data_cut_range_UP[ind] < self.on_mouse_press_coordinate_list_UP[1][0]):
                self.x_data_to_peak_fitting_UP.append(self.x_data_cut_range_UP[ind])
                self.y_data_to_peak_fitting_UP.append(y_normalized_data_cut_range[ind])
        ...
        initial_amplitude = 1
        initial_q = 1
        initial_x0 = (self.on_mouse_press_coordinate_list_UP[0][0] + self.on_mouse_press_coordinate_list_UP[1][0])/2
        initial_width = (self.on_mouse_press_coordinate_list_UP[0][0] - self.on_mouse_press_coordinate_list_UP[1][0])/2
        initial_params = (initial_amplitude, initial_q, initial_x0, initial_width)
        if self.PeakComboBoxUP.currentText() == "Fano resonance":
            self.params_of_peak_UP = peak_fitting_Fano_resonance(self.x_data_to_peak_fitting_UP,
                                                                 self.y_data_to_peak_fitting_UP,
                                                                 initial_params=initial_params)
            print(self.x_data_to_peak_fitting_UP)
            print(self.y_data_to_peak_fitting_UP)
            # если строить y_fitting_normalization_around_peak_UP вместо y_fitting_normalization_full_range
            # то будет строится моделированный пик не во всем диапазоне, а только вблизи пика
            # y_fitting_normalization_around_peak_UP = fitting_function(fano_resonance, self.params_of_peak_UP,self.x_data_to_peak_fitting_UP,plot=False)
            # self.axes_right.plot(self.x_data_to_peak_fitting_UP, y_fitting_normalization_around_peak_UP, color="red", linewidth=2)

            y_fitting_normalization_full_range = fitting_function(fano_resonance, self.params_of_peak_UP,
                                                                  self.x_data_cut_range_UP,
                                                                  plot=False)
            self.axes_right.plot(self.x_data_cut_range_UP, y_fitting_normalization_full_range, color="red",
                                linewidth=2)
    ...
    def save_fitting(self):
        ...
        # Условие, чтобы кнопка нажималась только после фитирования пика
        if len(self.params_of_peak_DN) > 0 or len(self.params_of_peak_UP) > 0:
    ...
    def save_to_file(self):
        ...
        # это можно сделать из self.export_data['header']
        header_for_saving = ",".join(self.export_list['header'])
        units_for_saving = "GHz, , , T, , , , T, , , , uV, , , , Ohm, , , , , , , , , , , , , , , , , , , , , , , \n"
    ...
    def export_single_results_to_list(self):
        ...
        if self.name_of_file_DN and self.name_of_file_UP:
            ...
            # на всякий случай сохраняем параметры пика и фона
            # a * ((e + q)**2) / (e**2 + 1) so a, q, x0, w = params_of_peak_DN[0], params_of_peak_DN[1], params_of_peak_DN[2], params_of_peak_DN[3]
            result_list.append("")
            result_list.append(self.params_of_peak_DN[0])
            result_list.append(self.params_of_peak_DN[1])
            result_list.append(self.params_of_peak_DN[2])
